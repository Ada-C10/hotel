...
def main
  ...
  until input.casecmp? "exit" || input == 7
    ...
    when "2", "Create a room block"
      create_block(admin)
    when "3", "List rooms available on date"
      list_avail_rooms(admin)
    when "4", "List reservations on date"
      list_res(admin)
    when "5", "Find an existing reservation"
      res_lookup(admin)
...
def main_menu
  ...
  rows << ["3", "List rooms available on date"]
  rows << ["4", "List reservations on date"]
  rows << ["5", "Find an existing reservation"]
  rows << ["6", "Find an existing block"]
  rows << ["7", "Exit"]

  table = Terminal::Table.new
  table.title = "HOTEL MANAGER: MAIN MENU\nCurrent availability: 2019-01-01 to 2019-12-31".colorize(:green)
...
def create_res(admin)
  begin
    ...
    print "Number of rooms (1-20): ".colorize(:blue)
    ...
    if block_res == "Y" || block_res == "YES"
      ...
      block = gets.chomp
      rows << ["Block name", block]
      res = admin.make_reservation(checkin_date, checkout_date, room_quantity, admin.find_block(block))
    else
